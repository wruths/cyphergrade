 ...
 * needed to backup one cypherlab->offline subplugin.
 *
 * Note: Offline cypherlabs really haven't any special subplugin
 ...
 * elements (cypherlab and submission) and conditions
 */
class backup_cypherlab_offline_subplugin extends backup_subplugin {

    /**
     * Returns the subplugin information to attach at cypherlab element
     */
    protected function define_cypherlab_subplugin_structure() {
        ...
        /**
         * Here we are defining the information that will be attached, within the "cypherlab" element
         * when cypherlabs of type "offline" are sent to backup, so we define the backup_subplugin_element
         * as not having any final element (null) and with the condition of the '/cypherlab/cypherlabtype'
         * being 'offline' (that will be checked on execution)
         *
         * Note that, while, we allow direct "injection" of final_elements at the "cypherlab" level (without
         ...
         */
        $subplugin = $this->get_subplugin_element(null, '/cypherlab/cypherlabtype', 'offline');
         ...
         * with cypherlabs. Take it as just one example. The only important bit is that it's highly recommended to
         * use some exclusive name in the main nested element (something that won't conflict with other subplugins/parts).
         * So we are using 'subplugin_cypherlab_offline_cypherlab' as name here (the type of the subplugin, the name of the
        ...
        $config->set_source_table('config', array('id' => '/cypherlab/id'));
